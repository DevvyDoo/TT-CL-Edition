...
class DistributedCashbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def handleSafeDropped(self, safe):
        ...
        battleTime = globalClock.getFrameTime() - self.battleThreeStart
        repositionDistance = self.progressValue(8, 28)  # Start at 8 units, progress to 28 units

        # First count how many safes are already nearby (always check within 35 units)
        checkDistance = 35
        nearbySafes = set()  # Using a set for faster lookup
        for potentialSafe in self.safes:
        ...
        safesNeeded = self.safesWanted - len(nearbySafes)
    ...
    def repositionSafe(self, safe, toonX, toonY, nearbyDistance):
        ...
        for _ in range(maxAttempts):
            ...
            if self.isLocationInBounds(x, y) and self.checkSafePosition(x, y, [s for s in self.safes if s != safe]):
    ...
    def setAttackCode(self, attackCode, avId=0):
        ...
        if attackCode in (ToontownGlobals.BossCogDizzy, ToontownGlobals.BossCogDizzyNow):
            ...
            if self.wantAlwaysStunned:
                delayTime = 3600 # please dont grind this for over an hour
            self.hitCount = 0
    ...
    def enterBattleThree(self):
        ...
        if self.wantAimPractice:
            self.setupAimMode()
            taskMgr.doMethodLater(1, self.stunLater, self.uniqueName('stun-later'))
    ...
    def stopCheckNearby(self):
        taskMgr.remove(self.uniqueName('checkNearby'))

    def __doCheckNearby(self, task):
        # Remove this method as it's no longer needed
        pass
