...
class DistributedCashbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def doorACallback(self, isOpen):
        if not hasattr(self, 'safes'):
            return
            
        # Find the door's actual node
        doorA = self.find('**/joint_doorFront')
        if not doorA:
            return
            
        # When door starts moving, activate physics for nearby safes
        doorPos = doorA.getPos(render)
        for safe in self.safes.values():
            if safe.state == 'Free':
                safePos = safe.getPos(render)
                
                # Get 2D distance ignoring Z (using XY only)
                distance = Vec2(doorPos[0] - safePos[0], 
                            doorPos[1] - safePos[1]).length()
                
                # Activate physics for safes within radius on XY plane
                if distance < 30:
                    print(f"Door A activating physics for safe {safe.doId} at distance {distance:.1f}")
                    safe.activatePhysics()
    ...
    def doorBCallback(self, isOpen):
        ...
        # Find the door's actual node    
        doorB = self.find('**/joint_doorRear')
        if not doorB:
            return
            
        # When door starts moving, activate physics for nearby safes
        doorPos = doorB.getPos(render)
        for safe in self.safes.values():
            if safe.state == 'Free':
                safePos = safe.getPos(render)
                ...
                # Activate physics for safes within radius on XY plane
                if distance < 30:
                    print(f"Door B activating physics for safe {safe.doId} at distance {distance:.1f}")
                    safe.activatePhysics()