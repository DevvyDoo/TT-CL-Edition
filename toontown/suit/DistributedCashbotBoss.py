...
class DistributedCashbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def enableBackWall(self):
        ...
        try:
            ...
            cn.setIntoCollideMask(
                OTPGlobals.WallBitmask | ToontownGlobals.PieBitmask | BitMask32.lowerOn(3) << 21)  # TTR Back Wall
        except:
    ...
    def makeIntroductionMovie(self, delayDeletes):
        ...
        track = Sequence(
            ...
            Wait(1),
            self.loseCogSuits(self.involvedToons, render, (113, -228, 10, 90, 0, 0)),
            Wait(.1),
            self.loseCogSuits([rToon], render, (113, -228, 10, 90, 0, 0), True),
            Wait(2),
            Func(camera.reparentTo, localAvatar),
            Func(camera.setPosHpr, 0, -12, 5, 0, -5, 0),
            Func(self.clearChat),
    ...
    def makePrepareBossRoundMovie(self, delayDeletes, crane, safe):
        ...
        for toonId in self.involvedToons:
            toon = self.cr.doId2do.get(toonId)
            if toon:
                delayDeletes.append(DelayDelete.DelayDelete(toon, 'CashbotBoss.makePrepareBossRoundMovie'))
        ...
        track = Sequence(
            ...
            self.moveToonsToBossRoundPos(self.getInvolvedToonsNotSpectating()),
            ...
            Func(camera.setPosHpr, 0, -12, 5, 0, -5, 0),
            Wait(1),
            Func(self.saySomething, '3'),
            Wait(1),
            Func(self.saySomething, '2'),
            Wait(1),
            Func(self.saySomething, '1'),
            Wait(1),
            Func(self.saySomething, 'time to game'),
        )

        return track

    def moveToonsToBossRoundPos(self, toons):
        track = Parallel()
    ...
    def makeBossFleeMovie(self):
        ...
        bossTrack = Track(
            ...
            (5.5, Parallel(
                ...
                Func(self.hideBossRoundObjects),
    ...
    def enterPrepareBossRound(self):
        ...
        intervalName = 'PrepareBossRoundMovie'
        delayDeletes = []
        ...
        seq = Sequence(self.makePrepareBossRoundMovie(delayDeletes, self.movieCrane, self.movieSafe),
                       Func(self.__beginBossRound), name=intervalName)
    ...
    def __beginBossRound(self):
        intervalName = 'PrepareBossRoundMovie'
        self.clearInterval(intervalName)

        self.doneBarrier('PrepareBossRound')
    ...
    def enterBossRound(self):
        ...
        DistributedBossCog.DistributedBossCog.enterBossRound(self)
        ...
        base.playMusic(self.bossRoundMusic, looping=1, volume=0.9)
    ...
    def exitBossRound(self):
        DistributedBossCog.DistributedBossCog.exitBossRound(self)
    ...
    def announceCraneRestart(self):
        ...
        title = OnscreenText(parent=aspect2d, text='All toons are sad!', style=3, fg=(.8, .2, .2, 1),
                             align=TextNode.ACenter, scale=.15, pos=(0, .35), font=ToontownGlobals.getCompetitionFont())
        sub = OnscreenText(parent=aspect2d, text=restartingOrEnding + 'crane round in 10 seconds...', style=3,
                           fg=(.8, .8, .8, 1), align=TextNode.ACenter, scale=.09, pos=(0, .2),
